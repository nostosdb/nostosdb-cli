...
The scripts do not publish, tag, sign with production credentials, create a Homebrew tap, or contact a package registry except when an operator explicitly runs npm tooling. The candidate workflow uploads review artifacts only and uses GitHub's attestation action as the provenance hook.
...
release formula and workflow still require distinct Apple Silicon and Intel evidence.
