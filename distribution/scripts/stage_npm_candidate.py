#!/usr/bin/env python3
"""Stage and pack the launcher plus one native npm candidate without publishing."""
...
def main() -> int:
    ...
    try:
        ...
        shutil.copytree(
            str(ROOT / "npm"),
            str(launcher_stage),
            ignore=shutil.ignore_patterns("packages", "tests", "scripts", "node_modules"),
        )
        ...
        if platform_result["name"] != details["npm_package"]:
            raise CandidateError("packed unexpected platform package")
