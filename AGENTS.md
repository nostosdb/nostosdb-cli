...
- Project writes update the root `.nostdb` first. When `nost` is true,
  finish the operation through `ProjectSynchronizer` so canonical `.nost` and
  its recorded synchronization baseline are reconciled.
