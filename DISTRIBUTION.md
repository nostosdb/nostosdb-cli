...
The implemented, unpublished `@nostosdb/server` global package belongs to the Server distribution and depends on this exact matching `@nostosdb/cli` package. It exposes both `nostosd` and `nostos`; this repository continues to own only the CLI launcher and `nostos` platform packages. No `@nostosdb/core` npm package is part of either installation.
